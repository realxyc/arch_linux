...
# if a previous run got partway through and left /mnt mounted, clean that
# up now — otherwise wipefs/mkfs on a reused partition number fails with
# "device or resource busy" and it's not obvious why.
if mountpoint -q /mnt; then
    echo "found /mnt still mounted from a previous run, unmounting first..."
    umount -R /mnt
fi
...
case "$MODE" in
    1)
        ...
        mkdir -p /mnt/boot
        mount -t vfat "$ARCH_ESP" /mnt/boot
        ;;
    2)
        mkdir -p /mnt/boot
        mount -t vfat "$WIN_ESP" /mnt/boot
        ;;
    3)
        mkdir -p /mnt/boot/efi
        mount -t vfat "$WIN_ESP" /mnt/boot/efi
        ;;
esac
